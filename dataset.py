...
class mtcnn_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        offset = torch.FloatTensor(item[2] if 4 == len(item[2]) else [0.0 for i in range(4)])
        landmark = torch.FloatTensor(item[3]) if 10 == len(item[3]) else torch.tensor([0.0 for i in range(10)])
        landmark_flag = torch.tensor([self.dict[item[1]]])
